(function () {
    ...
    async function fetchNewOpportunitiesForTrade(tradeId, neededCount) {
        return (
            OpportunityFetcher.fetchNewOpportunitiesForTrade?.(
                tradeId,
                neededCount,
                shouldStopCheck
            ) || []
    ...
    async function sendAllTrades() {
        if (isSendingAllTrades) {
            if (window.extensionAlert) {
                await window.extensionAlert(
                    'Already Sending',
                    'A trade sending process is already in progress. Please wait for it to complete or stop it first.',
                    'info'
                );
            }
            return;
        ...
        if (isAllTrades) {
            ...
            opportunitiesToSend = (
                ...
                []
            ).filter((opp) => String(opp.id) === String(tradeId));
        }
        if (opportunitiesToSend.length === 0) {
            if (window.extensionAlert) {
                await window.extensionAlert(
                    'No Trades',
                    'There are no trades available to send.',
                    'info'
                );
            }
            return;
        ...
        setTimeout(() => {
            window.SendAllProgressDialog.update(
                0,
                opportunitiesToSend.length,
                opportunitiesToSend,
                0,
                isAllTrades
            );
            if (window.SendAllProgressDialog && window.SendAllProgressDialog.addStatusLog) {
                window.SendAllProgressDialog.addStatusLog(
                    0,
                    3,
                    `Starting to send ${opportunitiesToSend.length} trade${opportunitiesToSend.length !== 1 ? 's' : ''}...`,
                    'info'
                );
            }
        }, 100);
        ...
        try {
            ...
            while (
                currentIndex < opportunitiesToSend.length ||
                attemptsWithoutProgress < maxAttemptsWithoutProgress
            ) {
                if (shouldStopCheck()) {
                    break;
                }
                const availableResult = progressTracker.hasAvailableOpportunity(
                    opportunitiesToSend,
                    currentIndex
                );
                ...
                if (!opportunity) {
                    ...
                    if (tradesNeedingMore.length > 0 && attemptsWithoutProgress < 3) {
                        ...
                        for (const [tradeId, goal] of tradesNeedingMore) {
                            ...
                            if (needed > 0) {
                                const newOpps = await fetchNewOpportunitiesForTrade(
                                    tradeId,
                                    needed
                                );
                                if (newOpps.length > 0) {
                        ...
                        if (fetchedNewOpportunities) {
                            ...
                            const totalSuccess = progressTracker.getTotalSuccess();
                            const totalGoal = progressTracker.getTotalGoal();
                            const opportunitiesToShow = progressTracker.getFilteredOpportunities(
                                opportunitiesToSend,
                                isAllTrades
                            );
                            window.SendAllProgressDialog.update(
                ...
                try {
                    tradeResult = await window.SendAllTradeSender.sendSingleTrade(
                        opportunity,
                    ...
                    if (
                        ...
                        window.SendAllChallengeHandler.isChallengeError(error)
                    ) {
                ...
                if (
                    challengeDetected ||
                    (tradeResult && tradeResult.reason === 'challenge_required')
                ) {
                    const progressText = progressDialog?.querySelector('#progress-text');
                    if (progressText) {
                    ...
                    const challengeCompleted =
                        await window.SendAllChallengeHandler.waitForCompletion(
                            12e4,
                            shouldStopCheck
                        );
                ...
                if (tradeResult && tradeResult.success) {
                    ...
                    if (!window.sentTrades.has(sentTradeKey)) {
                        ...
                        Storage.setAccount(
                            'sentTrades',
                            Array.from(window.sentTrades).map((key) => String(key))
                        );
                        Storage.flush();
                    }
                    window.currentOpportunities = (window.currentOpportunities || []).filter(
                        (opp) =>
                            !(
                                String(opp.id) === String(opportunity.id) &&
                                String(opp.targetUserId) === String(opportunity.targetUserId)
                            )
                    );
                    ...
                    const totalSuccess = progressTracker.getTotalSuccess();
                    const totalGoal = progressTracker.getTotalGoal();
                    const opportunitiesToShow = isAllTrades
                        ? opportunitiesToSend.filter((opp) => {
                              const key = `${String(opp.id)}-${String(opp.targetUserId)}`;
                              return !window.sentTrades.has(key);
                          })
                        : currentIndex < opportunitiesToSend.length
                          ? [opportunitiesToSend[currentIndex]]
                          : [];
                    window.SendAllProgressDialog.update(
                        totalSuccess,
                        totalGoal,
                    ...
                    const opportunitiesToShow = isAllTrades
                        ? opportunitiesToSend.filter((opp) => {
                              ...
                              const isRestricted = window.privacyRestrictedUsers.has(
                                  String(opp.targetUserId)
                              );
            ...
            if (currentAbortController === globalAbortController) {
                ...
                if (!stopWasPressed) {
                    ...
                    const opportunitiesToShow = isAllTrades
                        ...
                        : opportunitiesToSend.length > 0
                          ? [opportunitiesToSend[opportunitiesToSend.length - 1]]
                          : [];
                    window.SendAllProgressDialog.update(
                        totalSuccess,
                        totalGoal,
