(function () {
    ...
    async function loadTradeOpportunities() {
        ...
        const ownerPromises = tradesToLoad.map(({ trade: trade, itemIds: itemIds }) =>
            chrome.runtime
                .sendMessage({
        ...
        const processTradeResponse = async ({
            trade: trade,
            itemIds: itemIds,
            response: response,
        }) => {
            try {
                ...
                if (
                    realOwners.length > 0 &&
                    Array.isArray(realOwners[0]) &&
                    realOwners[0].length >= 3
                ) {
                ...
                if (remainingTrades > 0) {
                    ...
                    const storedPrivacyRestricted = await Storage.getAccountAsync(
                    ...
                    for (const userId of userIds) {
                        ...
                        const isPrivacyRestricted = window.privacyRestrictedUsers.has(
                            String(userId)
                        );
                    ...
                    function shuffleOwnersArray(array) {
                        ...
                        for (let fo = shuffled.length - 1; fo > 0; fo--) {
                            const j = Math.floor(Math.random() * (fo + 1));
                            [shuffled[fo], shuffled[j]] = [shuffled[j], shuffled[fo]];
                        }
                        return shuffled;
                    }
                    const shuffledOwners = shuffleOwnersArray(freshOwners);
                    const cap = Math.min(remainingTrades, shuffledOwners.length);
                    const ownersToShow = shuffledOwners.slice(0, cap);
        ...
        const storedPrivacyRestrictedFinal = await Storage.getAccountAsync(
            'privacyRestrictedUsers',
            []
        );
        if (storedPrivacyRestrictedFinal && Array.isArray(storedPrivacyRestrictedFinal)) {
            window.privacyRestrictedUsers = new Set(
                storedPrivacyRestrictedFinal.map((id) => String(id))
            );
        } else {
            window.privacyRestrictedUsers = new Set();
        }
        ...
        let filtered = opportunities.filter((opp) => {
            ...
            const isRestricted =
                window.privacyRestrictedUsers &&
                window.privacyRestrictedUsers.has(String(opp.targetUserId));
            return !isSent && !isRestricted;
        });
    ...
    window.OpportunitiesLoader = {
        loadTradeOpportunities: loadTradeOpportunities,
    };
