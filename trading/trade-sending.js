(function () {
    ...
    function setupSendTradeButtons() {
        document.querySelectorAll('.send-trade-btn').forEach((btn) => {
            btn.addEventListener('click', async (e) => {
                ...
                try {
                    const currentUserId =
                        Inventory.getCurrentUserId() || (await Inventory.getCurrentUserIdAsync());
                    if (!currentUserId) {
                        throw new Error('Could not get current user ID');
                    }
                    const ourItemIds = await Opportunities.getItemIdsFromTrade(
                        opportunity.giving,
                        window.rolimonData || {}
                    );
                    const theirItemIds = await Opportunities.getItemIdsFromTrade(
                        opportunity.receiving,
                        window.rolimonData || {}
                    );
                    if (
                        ourItemIds.length !== opportunity.giving.length ||
                        theirItemIds.length !== opportunity.receiving.length
                    ) {
                        throw new Error('Missing item IDs');
                    }
                    const mapAuto =
                        window.TradeItemIdAliases &&
                        window.TradeItemIdAliases.mapItemIdsForAutoInstanceApi
                            ? window.TradeItemIdAliases.mapItemIdsForAutoInstanceApi
                            : function (ids) {
                                  return ids;
                              };
                        ...
                        trade: [
                            {
                                ...
                                item_ids: mapAuto(ourItemIds),
                                robux: opportunity.robuxGive || 0,
                            },
                            {
                                user_id: userId,
                                item_ids: mapAuto(theirItemIds),
                    ...
                    if (
                        ...
                        !instanceResponse.data.participants
                    ) {
                    ...
                    if (
                        ourInstanceIds.length !== ourItemIds.length ||
                        theirInstanceIds.length !== theirItemIds.length
                    ) {
                    ...
                    try {
                        ...
                        if (!tradeResult) {
                            const angularTradeData = {
                                senderOffer: {
                                    ...
                                    robux: opportunity.robuxGive || 0,
                                    collectibleItemInstanceIds: Array.isArray(ourInstanceIds)
                                        ? ourInstanceIds
                                        : [],
                                },
                                recipientOffer: {
                                    userId: userId,
                                    robux: opportunity.robuxGet || 0,
                                    collectibleItemInstanceIds: Array.isArray(theirInstanceIds)
                                        ? theirInstanceIds
                                        : [],
                                },
                            };
                            try {
                                tradeResult = await BridgeUtils.callBridgeMethod(
                                    'sendTrade',
                                    angularTradeData
                                );
                            } catch (bridgeError) {
                                if (
                                    ...
                                    window.SendAllChallengeHandler.isChallengeError(bridgeError)
                                ) {
                                    e.target.dataset.trading = 'false';
                                ...
                                const hasPrivacyError =
                                    ...
                                    (bridgeError?.errors &&
                                        Array.isArray(bridgeError.errors) &&
                                        bridgeError.errors.some((e) => e.code === 22));
                                if (hasPrivacyError) {
                                    if (!window.privacyRestrictedUsers) {
                                        const stored =
                                            window.Storage?.getAccount(
                                                'privacyRestrictedUsers',
                                                []
                                            ) || [];
                                        window.privacyRestrictedUsers = new Set(
                                            stored.map((id) => String(id))
                                        );
                                    }
                                    window.privacyRestrictedUsers.add(String(userId));
                                    if (window.Storage) {
                                        window.Storage.setAccount(
                                            'privacyRestrictedUsers',
                                            Array.from(window.privacyRestrictedUsers).map((id) =>
                                                String(id)
                                            )
                                        );
                                    }
                                    window.currentOpportunities = (
                                        window.currentOpportunities || []
                                    ).filter((opp) => String(opp.targetUserId) !== String(userId));
                        ...
                        if (tradeResult && tradeResult.errors) {
                            ...
                            if (hasPrivacyError) {
                                if (!window.privacyRestrictedUsers) {
                                    const stored =
                                        window.Storage?.getAccount('privacyRestrictedUsers', []) ||
                                        [];
                                    window.privacyRestrictedUsers = new Set(
                                        stored.map((id) => String(id))
                                    );
                        ...
                        if (tradeResult && tradeResult.tradeId) {
                            if (TradeStorage && TradeStorage.saveTradeAfterSending) {
                                await TradeStorage.saveTradeAfterSending(
                                    tradeResult,
                                    opportunity,
                                    tradeId,
                                    userId
                                ...
                                window.TradeStorage &&
                                window.TradeStorage.saveTradeAfterSending
                            ) {
                                await window.TradeStorage.saveTradeAfterSending(
                                    tradeResult,
                                    opportunity,
                                    tradeId,
                                    userId
    ...
    window.TradeSending = {
        setupSendTradeButtons: setupSendTradeButtons,
        copyChallengeToPrimaryPage: copyChallengeToPrimaryPage,
        showOriginalRobloxInterface: showOriginalRobloxInterface,
    };
