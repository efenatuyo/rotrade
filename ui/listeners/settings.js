(function () {
    ...
    function setupSettingsEventListeners() {
        ...
        if (saveBtn) {
            saveBtn.addEventListener('click', async () => {
                const maxOwnerDays = parseInt(document.getElementById('maxOwnerDays').value) || 1e8;
                const lastOnlineDays =
                    parseInt(document.getElementById('lastOnlineDays').value) || 3;
                const tradeMemoryDays =
                    parseInt(document.getElementById('tradeMemoryDays').value) || 7;
                if (maxOwnerDays < 8 || maxOwnerDays > 999999999) {
                    Dialogs.alert(
                        'Invalid Value',
                        'Max Owner Days must be between 8 and 999,999,999',
                        'error'
                    );
                    return;
                }
                if (lastOnlineDays < 1 || lastOnlineDays > 365) {
                    Dialogs.alert(
                        'Invalid Value',
                        'Last Online Days must be between 1 and 365',
                        'error'
                    );
                    return;
                }
                if (tradeMemoryDays < 1 || tradeMemoryDays > 30) {
                    Dialogs.alert(
                        'Invalid Value',
                        'Trade Memory Days must be between 1 and 30',
                        'error'
                    );
                    return;
                }
        ...
        if (resetBtn) {
            resetBtn.addEventListener('click', async () => {
                ...
                if (confirmed) {
                    document.getElementById('maxOwnerDays').value = 1e8;
        ...
        if (clearHistoryBtn) {
            clearHistoryBtn.addEventListener('click', async () => {
                const confirmed = await Dialogs.confirm(
                    'Clear Trade History',
                    'Are you sure you want to clear all sent trade history? This will allow you to send trades to users again immediately.',
                    'Clear History',
                    'Cancel'
                );
                if (confirmed) {
                ...
                try {
                    ...
                    if (!base32Regex.test(secret)) {
                        Dialogs.alert(
                            ...
                            'The secret key must contain only uppercase letters A-Z and numbers 2-7 (Base32 format). Please check and try again.',
                            'error'
                        );
                        return;
                    }
                    if (secret.length < 16) {
                        Dialogs.alert(
                            ...
                            'The secret key appears to be too short. Please check that you copied the entire code.',
                            'error'
                        );
                        return;
                    }
                    ...
                    try {
                        ...
                        if (!testCode || testCode.length !== 6 || !/^\d{6}$/.test(testCode)) {
                            ...
                            Dialogs.alert(
                                'Invalid Secret',
                                'The secret key format is invalid and cannot generate TOTP codes. Please check and try again.',
                                'error'
                            );
                        ...
                        Dialogs.alert(
                            'Success',
                            '2FA secret has been validated and saved securely. Auto-confirmer can now be enabled.',
                            'info'
                        );
                    } catch (error) {
                        if (statusText) {
                        ...
                        if (error.message && error.message.includes('Invalid secret format')) {
                            Dialogs.alert(
                                'Invalid Secret Format',
                                'The secret key format is invalid. Please ensure you copied the entire code correctly from Roblox.',
                                'error'
                            );
                        } else {
                            Dialogs.alert(
        ...
        if (reset2FASecretBtn) {
            reset2FASecretBtn.addEventListener('click', async () => {
                const confirmed = await Dialogs.confirm(
                    'Reset 2FA Secret',
                    'Are you sure you want to reset your 2FA secret? You will need to set it again to use auto-confirmer.',
                    'Reset',
                    'Cancel'
                );
                if (confirmed) {
                        ...
                        if (userId) {
                            ...
                            if (statusText) {
                                ...
                                statusText.style.color =
                                    'var(--auto-trades-text-secondary, #bdbebe)';
                            }
    ...
    window.EventListenersSettings = {
        setupSettingsEventListeners: setupSettingsEventListeners,
        update2FAStatus: update2FAStatus,
    };
