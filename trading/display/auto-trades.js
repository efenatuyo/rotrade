(function () {
    ...
    const {
        ...
        TradeDisplayCore: TradeDisplayCore,
    ...
    function displayAutoTrades(autoTrades) {
        ...
        if (!container) return;
        const isVisible =
            autoTradesSection && window.getComputedStyle(autoTradesSection).display !== 'none';
        ...
        container.style.display = 'grid';
        if (emptyState) emptyState.style.display = 'none';
        const cards = autoTrades
            .map((autoTrade) => {
                const values = TradeDisplayCore.calcValues(autoTrade);
                const maxTrades =
                    autoTrade.settings?.maxTrades || autoTrade.settings?.maxTradesPerDay || 5;
        ...
        if (
            ...
            window.TradeDisplayActions.handleAutoTradeActions
        ) {
            autoTradesContainer.removeEventListener(
                'click',
                window.TradeDisplayActions.handleAutoTradeActions
            );
            autoTradesContainer.addEventListener(
                'click',
                window.TradeDisplayActions.handleAutoTradeActions
            );
        }
    ...
    window.TradeDisplayAutoTrades = {
        displayAutoTrades: displayAutoTrades,
    };
