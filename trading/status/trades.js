(function () {
    ...
    function processStatusUpdates(pendingTrades, tradeStatusMap) {
        ...
        for (const trade of pendingTrades) {
            ...
            if (!robloxStatus && tradeNorm) {
                robloxStatus =
                    tradeStatusMap.get(tradeNorm.str) || tradeStatusMap.get(tradeNorm.num);
            }
            if (!robloxStatus) {
            ...
            if (normalizedStatus === 'open') {
                ...
                if (existingIndex >= 0) {
                    ...
                    const statusChanged =
                        (existingTrade.status || '').toLowerCase() !== normalizedStatus;
                    finalizedTrades[existingIndex] = finalizedTrade;
                    if (statusChanged) {
    ...
    function notifyAndRefreshUI(movedTrades) {
        ...
        if (
            typeof TradeLoading.loadExpiredTrades === 'function' &&
            document.getElementById('expired-container')
        ) {
            TradeLoading.loadExpiredTrades();
        }
        if (
            typeof TradeLoading.loadCompletedTrades === 'function' &&
    ...
    window.TradeStatusTrades = {
        moveTradeToFinalized: moveTradeToFinalized,
        processStatusUpdates: processStatusUpdates,
        notifyAndRefreshUI: notifyAndRefreshUI,
    };
