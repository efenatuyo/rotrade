(function () {
                    ...
                    (response) => {
                        ...
                        if (response?.success && response.data?.items) {
                            ...
                            if (typeof items !== 'object' || items === null) {
                                Utils.Logger.log('fetch_rolimons_invalid_items', {
                                    type: typeof items,
                                });
                                resolve(Utils.ensureArray(rolimonsCache.data, []));
                                return;
            ...
            do {
                ...
                if (data.data?.length > 0) {
                    const pageItems = data.data.map((item) => {
                        const rolimonItem = Object.values(rolimonData).find(
                            (r) => r[0] === item.name
                        );
                        const rap = rolimonItem ? rolimonItem[2] : item.recentAveragePrice || 1e3;
                        const value = rolimonItem ? rolimonItem[4] : item.recentAveragePrice || 1e3;
                        return {
                            name: item.name,
                            id: item.assetId,
                            value: value,
                            ...
                            copies: 1,
                            rarity: value > 5e4 ? 'legendary' : value > 1e4 ? 'rare' : 'common',
                        };
                    });
    ...
    async function fetchUsernames(userIds) {
        ...
        if (!result.ok) {
            Utils.Logger.log('fetch_usernames_failed', {
    ...
    window.API = {
        fetchRolimons: fetchRolimons,
        getCurrentUserId: getCurrentUserId,
        getCurrentUserIdSync: getCurrentUserIdSync,
        clearUserIdCache: clearUserIdCache,
        getUserCollectibles: getUserCollectibles,
        fetchCommonOwners: fetchCommonOwners,
        fetchPlayerAssets: fetchPlayerAssets,
