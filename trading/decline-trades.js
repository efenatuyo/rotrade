(function () {
    ...
    async function processDeclineTrades(
        autoTradeId,
        tradesToDecline,
        initialDeclined = 0,
        initialFailed = 0,
        ...
        if (isDeclining) {
            return {
                ...
                total: totalCount || tradesToDecline.length,
            };
        }
        isDeclining = true;
        ...
        const progressDialogModule =
            window.DeclineProgressDialog || window.ModuleRegistry?.getSafe('DeclineProgressDialog');
        ...
        try {
            while (remainingTrades.length > 0) {
                ...
                while (!success && tradeStillPending) {
                    ...
                    try {
                        ...
                        if (result.success) {
                            ...
                            const finalizedTrades = Storage.getAccount(
                            ...
                            remainingTrades.shift();
                            const progressDialogModule =
                                window.DeclineProgressDialog ||
                                window.ModuleRegistry?.getSafe('DeclineProgressDialog');
                            if (progressDialogModule && progressDialogModule.update) {
                                progressDialogModule.update(declined, total, failed);
                            }
                ...
                if (!success && !tradeStillPending) {
                    ...
                    !success &&
                    tradeStillPending &&
                    (shouldStopDeclining || currentDeclineController.signal.aborted)
                ) {
                    break;
                }
            }
    ...
    async function declineMatchingOutboundTrades(autoTradeId) {
        const pendingTrades = Storage.getAccount('pendingExtensionTrades', []);
        const matchingTrades = pendingTrades.filter(
            (trade) =>
                trade.autoTradeId === autoTradeId ||
    ...
    window.DeclineTrades = {
        ...
        declineMatchingOutboundTrades: declineMatchingOutboundTrades,
    };
})();
