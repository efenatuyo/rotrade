(function () {
        ...
        try {
            ...
            if (canTradeData.canTrade === false) {
                ...
                const friendlyMessage =
                    statusMessages[canTradeData.status] ||
                    canTradeData.status ||
                    'Cannot trade with this user';
                return {
                    canTrade: false,
    ...
    async function validateTradeItems(opportunity) {
        try {
            ...
            if (ourItemIds.length !== opportunity.giving.length) {
                throw new Error(
                    `Missing item IDs for giving items. Expected ${opportunity.giving.length}, got ${ourItemIds.length}`
                );
            }
            if (theirItemIds.length !== opportunity.receiving.length) {
                throw new Error(
                    `Missing item IDs for receiving items. Expected ${opportunity.receiving.length}, got ${theirItemIds.length}`
                );
            }
            return {
                valid: true,
                currentUserId: currentUserId,
                ourItemIds: ourItemIds,
                theirItemIds: theirItemIds,
            };
        } catch (error) {
            if (window.handleUnexpectedError) {
                window.handleUnexpectedError(error, {
    ...
    async function validateInstanceIds(
        instanceResponse,
        ourItemIds,
        theirItemIds,
        currentUserId,
        ...
        try {
            ...
            const ourInstanceIds = (ourTradeData?.item_instance_ids || []).slice(
                0,
                ourItemIds.length
            );
            const theirInstanceIds = (theirTradeData?.item_instance_ids || []).slice(
                0,
                theirItemIds.length
            );
            if (ourInstanceIds.length !== ourItemIds.length) {
                throw new Error(
                    `Missing instance IDs for giving items. Expected ${ourItemIds.length}, got ${ourInstanceIds.length}`
                );
            }
            if (theirInstanceIds.length !== theirItemIds.length) {
                throw new Error(
                    `Missing instance IDs for receiving items. Expected ${theirItemIds.length}, got ${theirInstanceIds.length}`
                );
            }
            return {
                valid: true,
                ourInstanceIds: ourInstanceIds,
                theirInstanceIds: theirInstanceIds,
            };
        } catch (error) {
            if (window.handleUnexpectedError) {
                window.handleUnexpectedError(error, {
    ...
    window.TradeValidator = {
        validateTradeCanBeSent: validateTradeCanBeSent,
        validateTradeItems: validateTradeItems,
        validateInstanceIds: validateInstanceIds,
    };
})();
