(function () {
    ...
    async function checkRobloxTradeStatuses() {
        try {
            ...
            const validTradesToCheck = tradesToCheck.filter(
                (tid) => tid && tid !== 'undefined' && tid !== 'null'
            );
            if (validTradesToCheck.length > 0 && fetcher.fetchStatusForChangedTrades) {
                const onStatusFound =
                    TradeStatusChecker.createStatusFoundCallback?.(
                        tradeStatusMap,
                        pendingTradesMap
                    ) || (() => {});
                ...
                const normalizeTradeId =
                    window.TradeIdNormalizer?.normalizeTradeId ||
                    ((id) => {
                    ...
                    if (status && status.trim()) {
                        ...
                        if (!isInList) {
                            for (const foundId of foundInPaginatedList) {
                                const tradeIdsMatch =
                                    window.TradeIdNormalizer?.tradeIdsMatch || (() => false);
            ...
            const {
                ...
                finalizedTrades: finalizedTrades,
                movedTrades: movedTrades,
            } = processStatusUpdates(pendingTrades, tradeStatusMap);
    ...
    window.TradeStatusRoblox = {
        checkRobloxTradeStatuses: checkRobloxTradeStatuses,
    };
