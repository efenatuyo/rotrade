(function () {
    ...
    function cleanupTradeCategories() {
        ...
        let moveCount = 0;
        const toMove = pendingTrades.filter(
            (trade) =>
                trade.status &&
                ['declined', 'accepted', 'completed', 'expired'].includes(trade.status)
        );
        if (toMove.length > 0) {
