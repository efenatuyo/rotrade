(function () {
    ...
    async function callBridgeMethod(action, data, timeout = null) {
        return new Promise((resolve, reject) => {
            ...
            window.dispatchEvent(
                new CustomEvent('extensionBridgeRequest', {
                    detail: {
                        action: action,
                        data: data,
                        requestId: requestId,
            ...
            const timeoutMs = timeout || (action === 'sendTrade' ? 3e4 : 5e3);
            setTimeout(() => {
                window.removeEventListener('extensionBridgeResponse', handler);
    ...
    const CSRF_TOKEN_CACHE_DURATION = 60 * 1e3;
    ...
    window.BridgeUtils = {
        setupPageContextBridge: setupPageContextBridge,
        callBridgeMethod: callBridgeMethod,
        waitForAngularViaBridge: waitForAngularViaBridge,
        getRobloxCSRFToken: getRobloxCSRFToken,
        waitForPageAndAngularReady: waitForPageAndAngularReady,
        waitForAngularAndCache: waitForAngularAndCache,
    };
