(function () {
        ...
        if (e.target.classList.contains('delete-auto-trade')) {
            ...
            if (tradeId) {
                ...
                if (result && (result === true || result.confirmed)) {
                    ...
                    const shouldDecline =
                        result && typeof result === 'object' && result.checkbox === true;
                    if (shouldDecline) {
                        ...
                        if (!DeclineTrades) {
                            if (window.extensionAlert) {
                                window.extensionAlert(
                                    'Error',
                                    'DeclineTrades module not available. Please refresh the page.',
                                    'error'
                                );
                            }
                            return;
                        ...
                        if (!DeclineTrades.declineMatchingOutboundTrades) {
                            if (window.extensionAlert) {
                                window.extensionAlert(
                                    'Error',
                                    'declineMatchingOutboundTrades function not available',
                                    'error'
                                );
                            }
                            return;
                        ...
                        try {
                            DeclineTrades.declineMatchingOutboundTrades(tradeId)
                                .then((declineResult) => {
                            ...
                            if (window.extensionAlert) {
                                window.extensionAlert(
                                    'Error',
                                    'Failed to call decline function: ' +
                                        (error.message || 'Unknown error'),
                                    'error'
                                );
                            }
            ...
            if (tradeId) {
                ...
                if (tradeToEdit) {
                    ...
                    const path = window.Routing
                        ? window.Routing.buildPath('/auto-trades/create')
                        : '/auto-trades/create';
    ...
    window.TradeDisplayActions = {
        handleAutoTradeActions: handleAutoTradeActions,
    };
})();
