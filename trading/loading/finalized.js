(function () {
    ...
    function formatFinalizedTrade(trade, rolimonData, statusFilter) {
        ...
        if (Object.keys(rolimonData).length > 0 && createRolimonsIndex && enrichItemWithRolimons) {
            const { nameIndex: nameIndex } = createRolimonsIndex(rolimonData);
            for (let i = 0; i < giving.length; i++) {
                const enriched = enrichItemWithRolimons(giving[i], nameIndex);
                if (enriched.rap)
                    giving[i] = {
                        ...giving[i],
                        rap: giving[i].rap || enriched.rap,
                    };
                if (enriched.value)
                    giving[i] = {
                        ...giving[i],
                        value: giving[i].value || enriched.value,
                    };
                if (enriched.id)
                    giving[i] = {
                        ...giving[i],
            ...
            for (let i = 0; i < receiving.length; i++) {
                ...
                if (enriched.rap)
                    receiving[i] = {
                        ...receiving[i],
                        rap: receiving[i].rap || enriched.rap,
                    };
                if (enriched.value)
                    receiving[i] = {
                        ...receiving[i],
                        value: receiving[i].value || enriched.value,
                    };
                if (enriched.id)
                    receiving[i] = {
                        ...receiving[i],
        ...
        const expiredTrades = finalizedTrades
            .filter((trade) => {
                ...
                return (
                    status === 'expired' ||
                    status === 'declined' ||
                    robloxStatus === 'expired' ||
                    status === 'countered'
                );
            })
            .map((trade) => {
                const status = (trade.status || '').toLowerCase();
                const filterKey = status === 'countered' ? 'countered' : 'expired';
                return formatFinalizedTrade(trade, rolimonData, filterKey);
            });
    ...
    window.TradeLoadingFinalized = {
        loadExpiredTrades: loadExpiredTrades,
