(function () {
    ...
    function validateRobuxLimits(robuxGive, robuxGet) {
        ...
        const totalGivingValue = Array.from(selectedInventory).reduce(
            (sum, item) => sum + (parseInt(item.dataset.value) || 0),
            0
        );
        ...
        if (totalGivingValue > 0 && robuxGive > 0) {
            ...
            if (robuxGive > maxRobuxGive) {
                Dialogs.alert(
                    ...
                    `Robux you give (${robuxGive.toLocaleString()}) cannot exceed 50% of your offering items' total value (${totalGivingValue.toLocaleString()}). Maximum allowed: ${Math.floor(maxRobuxGive).toLocaleString()} Robux.`,
                    'error'
                );
                return false;
            }
        ...
        if (totalReceivingValue > 0 && robuxGet > 0) {
            ...
            if (robuxGet > maxRobuxGet) {
                Dialogs.alert(
                    'Invalid Robux Amount',
                    `Robux you want (${robuxGet.toLocaleString()}) cannot exceed 50% of the receiving items' total value (${totalReceivingValue.toLocaleString()}). Maximum allowed: ${Math.floor(maxRobuxGet).toLocaleString()} Robux.`,
                    'error'
                );
                return false;
            }
