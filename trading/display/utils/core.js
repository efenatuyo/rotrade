(function () {
    ...
    const getStatusConfig = (status, containerId) => {
        ...
        const isHist =
            containerId.includes('completed') || containerId.includes('expired');
        const map = {
            declined: {
                color: '#dc3545',
    ...
    const getPagItems = (items, perPage, curr) => {
        const { startIndex: startIndex, endIndex: endIndex } = calcPag(items.length, perPage, curr);
    ...
    const getEmptyMsg = (id) => {
        ...
        if (id.includes('expired')) return 'No declined or countered trades found.';
        return 'No trades found.';
    };
    window.TradeDisplayCore = {
        formatNum: formatNum,
        formatRobux: formatRobux,
        ...
        getPagKeys: getPagKeys,
        getPage: getPage,
        setPage: setPage,
        getSort: getSort,
        setSort: setSort,
        calcPag: calcPag,
        getPagItems: getPagItems,
