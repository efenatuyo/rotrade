(function () {
    ...
    function createAutoTrade() {
        ...
        const selectedCatalog = document.querySelectorAll(
            '#catalog-grid .item-card[data-quantity]:not([data-quantity="0"])'
        );
        const totalCatalogItems = Array.from(selectedCatalog).reduce(
            (total, item) => total + (parseInt(item.dataset.quantity) || 0),
            0
        );
        if (selectedInventory.length === 0 || totalCatalogItems === 0) {
            Dialogs.alert(
                'Selection Required',
                'Please select items from both your inventory and the catalog',
                'error'
            );
            return;
        }
        ...
        if (totalCatalogItems > 4) {
            Dialogs.alert(
                'Too Many Items',
                'You can only receive up to 4 items total. You currently have ' +
                    totalCatalogItems +
                    ' items selected.',
                'error'
            );
        ...
        const autoTradeData = {
            ...
            receiving: Array.from(selectedCatalog)
                .flatMap((item) => {
        ...
        if (isEditMode) {
            const tradeIndex = autoTrades.findIndex(
                (trade) => trade.id === tradeId || String(trade.id) === String(tradeId)
            );
            if (tradeIndex !== -1) {
    ...
    async function deleteAutoTrade(id) {
        ...
        const matchingCount = pendingTrades.filter(
            (trade) => trade.autoTradeId === id || String(trade.autoTradeId) === String(id)
        ).length;
        const checkboxLabel =
            matchingCount > 0
        ...
        if (result && (result === true || result.confirmed)) {
            ...
            if (
                ...
                DeclineTrades &&
                DeclineTrades.declineMatchingOutboundTrades
            ) {
    ...
    window.TradeOperations = {
        createAutoTrade: createAutoTrade,
        deleteAutoTrade: deleteAutoTrade,
    };
