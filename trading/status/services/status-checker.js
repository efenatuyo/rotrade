(function () {
    ...
    function normalizeTradeId(id) {
        const normalizer =
            TradeIdNormalizer.normalizeTradeId ||
            ((id) => {
    ...
    function tradeIdsMatch(id1, id2) {
        const matcher =
            TradeIdNormalizer.tradeIdsMatch ||
            ((id1, id2) => {
    ...
    async function checkTradeStatuses(pendingTrades, fetcher) {
        ...
        const foundInPaginatedList = fetcher?.findPendingTradesInPaginatedList
            ? await fetcher.findPendingTradesInPaginatedList(pendingTrades, oldestPendingTime)
            : new Set();
        ...
        for (const tradeId of pendingTradeIds) {
            ...
            if (!isInList) {
                for (const foundId of foundInPaginatedList) {
                    ...
                    if (
                        foundIdNorm &&
                        (tradeNorm.str === foundIdNorm.str ||
                            tradeNorm.num === foundIdNorm.num ||
                            tradeNorm.str === foundIdNorm.num ||
                            tradeNorm.num === foundIdNorm.str)
                    ) {
                        isInList = true;
                        break;
                    }
        ...
        for (const trade of pendingTrades) {
            ...
            let foundInList =
                foundInPaginatedList.has(tradeNorm.str) ||
                foundInPaginatedList.has(tradeNorm.num) ||
                tradeStatusMap.has(tradeNorm.str) ||
                tradeStatusMap.has(tradeNorm.num);
            if (!foundInList) {
                for (const foundId of foundInPaginatedList) {
        ...
        return {
            ...
            tradesToCheck: tradesToActuallyCheck,
            foundInPaginatedList: foundInPaginatedList,
    ...
    window.TradeStatusChecker = {
        checkTradeStatuses: checkTradeStatuses,
        createStatusFoundCallback: createStatusFoundCallback,
    };
})();
