(function () {
    ...
    function checkForEditMode() {
        ...
        try {
            ...
            const nameInput =
                document.getElementById('auto-trade-name') ||
            ...
            if (tradeData.giving && tradeData.giving.length > 0) {
                const selectInventoryItems = () => {
                    ...
                    Object.entries(itemCounts).forEach(([itemName, requiredCount]) => {
                        let inventoryCards = Array.from(
                            document.querySelectorAll(`#inventory-grid [data-item="${itemName}"]`)
                        );
                        if (inventoryCards.length === 0) {
                            inventoryCards = Array.from(
                                document.querySelectorAll(`#inventory-grid [title="${itemName}"]`)
                            );
                        }
                        if (inventoryCards.length === 0) {
                            inventoryCards = Array.from(
                                document.querySelectorAll(
                                    `#inventory-grid [data-name="${itemName}"]`
                                )
                            );
                        }
                        if (inventoryCards.length > 0) {
                            const availableCards = inventoryCards.filter(
                                (card) => card.dataset.onHold !== 'true'
                            );
                    ...
                    const selectedInventoryItems = document.querySelectorAll(
                        '#inventory-grid .item-card.selected'
                    );
            ...
            if (targetItems && targetItems.length > 0) {
                const selectCatalogItems = () => {
                    ...
                    Object.entries(catalogItemCounts).forEach(([itemName, requiredCount]) => {
                        let catalogCard =
                            document.querySelector(`#catalog-grid [data-item="${itemName}"]`) ||
                        ...
                        if (catalogCard) {
                            ...
                            catalogCard.style.setProperty(
                                '--quantity-number',
                                `"${Math.min(requiredCount, 4)}"`
                            );
                        }
                    });
                    if (window.updateTradeSummaryGlobal) {
                    ...
                    const selectedCatalogItems = document.querySelectorAll(
                        '#catalog-grid .item-card.selected'
                    );
            ...
            if (
                tradeData.settings &&
                (tradeData.settings.maxTradesPerDay || tradeData.settings.maxTrades)
            ) {
                const maxTradesInput = document.getElementById('max-trades');
                if (maxTradesInput) {
                    const maxTradesValue =
                        tradeData.settings.maxTradesPerDay || tradeData.settings.maxTrades || 5;
            ...
            requestAnimationFrame(() => {
                const allSelectedItems = document.querySelectorAll(
                    '#inventory-grid .item-card.selected, #catalog-grid .item-card.selected'
                );
    ...
    window.debugEditMode = function () {
        ...
        return {
            inventoryCards: inventoryCards.length,
