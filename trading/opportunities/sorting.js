(function () {
    ...
    function applySort() {
        ...
        window.filteredOpportunities.sort((a, b) => {
            const rawDataA =
                window.ownersRawData &&
                window.ownersRawData[a.id]?.find((u) => u.userId === a.targetUserId);
            const rawDataB =
                window.ownersRawData &&
                window.ownersRawData[b.id]?.find((u) => u.userId === b.targetUserId);
            let valA = 0;
            let valB = 0;
    ...
    window.OpportunitiesSorting = {
        setupSortingSystem: setupSortingSystem,
        applySort: applySort,
    };
