(function () {
    'use strict';
    const { SecurityUtils: SecurityUtils, TradeDisplayCore: TradeDisplayCore } = window;
    ...
    const tooltipPlain = (i) => {
        const name = normName(i);
        const rap = TradeDisplayCore.formatNum(i.rap || 0);
        const val = TradeDisplayCore.formatNum(i.value || 0);
        return `${name}&#10;RAP ${rap}&#10;VAL ${val}`;
    };
    ...
    const opportunityItemCache = new WeakMap();
    const renderOpportunityItem = (item, variant = 'default') => {
        if (!item || typeof item !== 'object') return '';
        const cacheKey = `${normId(item)}:${variant}`;
        const cachedMap = opportunityItemCache.get(item) || new Map();
        if (cachedMap.has(cacheKey)) {
            return cachedMap.get(cacheKey);
        }
        const id = String(normId(item));
        const short = shortName(item);
        const tip = tooltipPlain(item);
        const sname = String(item.name || '');
        const displayName = normName(item);
        const truncatedName =
            displayName.length > 15 ? displayName.substring(0, 15) + '...' : displayName;
        const html =
            variant === 'compact'
                ? `<div class="item-card-compact"><div class="item-icon" data-item-id="${id}" data-id="${id}" data-item-name="${sname}" title="${tip}">${short}</div><div class="item-info-compact"><div class="item-name-compact">${truncatedName}</div><div class="item-values-compact"><span class="rap-text">RAP: ${TradeDisplayCore.formatNum(item.rap || 0)}</span><span class="value-text">VAL: ${TradeDisplayCore.formatNum(item.value || 0)}</span></div></div></div>`
                : `<div class="item-icon" data-item-id="${id}" data-id="${id}" data-item-name="${sname}" title="${tip}">${short}</div>`;
        cachedMap.set(cacheKey, html);
        opportunityItemCache.set(item, cachedMap);
        return html;
    };
    ...
    const renderOpportunityItems = (items, robux = 0, variant = 'default') => {
        const itemsArray = Array.isArray(items) ? items : [];
        return (
            itemsArray.map((i) => renderOpportunityItem(i, variant)).join('') +
            renderRobux(robux, variant)
        );
    };
    ...
    const renderSection = (title, items, robux, variant = 'default') =>
        `<div class="items-section"><div class="items-title">${title}</div><div class="items-list">${renderItems(items, robux, variant)}</div></div>`;
    ...
    const renderSimpleCard = (trade) => {
        const sc = trade.status?.includes('Pending')
            ? '#ffc107'
    ...
    const renderCard = (trade, containerId) =>
        TradeDisplayCore.hasItems(trade)
            ? renderFullCard(trade, containerId)
            : renderSimpleCard(trade);
    window.TradeDisplayRenderer = {
        renderItem: renderItem,
        renderOpportunityItem: renderOpportunityItem,
        renderRobux: renderRobux,
        renderItems: renderItems,
        renderOpportunityItems: renderOpportunityItems,
        renderHeader: renderHeader,
        renderSection: renderSection,
        renderValues: renderValues,
        renderFullCard: renderFullCard,
        renderSimpleCard: renderSimpleCard,
