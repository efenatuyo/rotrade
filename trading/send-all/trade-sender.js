(function () {
    ...
    async function sendSingleTrade(opportunity, abortSignal, shouldStopCheck, onStatusUpdate) {
        ...
        const mapAuto =
            window.TradeItemIdAliases && window.TradeItemIdAliases.mapItemIdsForAutoInstanceApi
        ...
        if (window.AutoConfirmer) {
            ...
            if (autoConfirmerResult && autoConfirmerResult.success) {
                ...
                const yourIds = await Opportunities.getItemIdsFromTrade(
                    opportunity.giving,
                    window.rolimonData || {}
                );
                const theirIds = await Opportunities.getItemIdsFromTrade(
                    opportunity.receiving,
                    window.rolimonData || {}
                );
                    ...
                    if (storedTrade) {
                        ...
                        if (newCount >= maxTrades) {
                            ...
                            window.currentOpportunities = (
                                ...
                                (opp) =>
                                    !(
                                        String(opp.id) === String(tradeId) &&
                                        String(opp.targetUserId) === String(userId)
                                    )
                            );
                            window.filteredOpportunities = (
                                window.filteredOpportunities || []
            ...
            if (autoConfirmerResult && autoConfirmerResult.useFallback) {
                ...
                if (autoConfirmerResult.noFallback) {
                    return {
                        success: false,
                        reason: 'auto_confirmer_failed',
                        error: autoConfirmerResult.error,
                    };
                }
        ...
        try {
            const tradeResult = await BridgeUtils.callBridgeMethod(
                'sendTrade',
                angularTradeData,
                2e4
            );
                ...
                if (hasPrivacyError) {
                    ...
                    if (onStatusUpdate) {
                        onStatusUpdate(
                            3,
                            3,
                            `Privacy settings prevent trading with ${username}`,
                            'error'
                        );
                    }
                    return {
            ...
            if (onStatusUpdate) {
                onStatusUpdate(
                    3,
                    3,
                    `Failed to send trade to ${username}: ${errorMessage}`,
                    'error'
                );
            }
            return {
