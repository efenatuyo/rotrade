(function () {
    'use strict';
    async function waitForChallengeCompletion(maxWaitTime = 12e4, shouldStopCheck) {
        ...
        while (Date.now() - startTime < maxWaitTime) {
            ...
            const challengeModal = document.querySelector(
                '.modal-overlay, .modal-container, [class*="challenge"], [class*="verification"]'
            );
            const challengeIframe = document.querySelector(
                'iframe[src*="challenge"], iframe[src*="verification"]'
            );
    ...
    function isChallengeError(error) {
        ...
        if (error?.status) {
            ...
            const statusMatch =
                errorMessage.match(/status[:\s]+(\d+)/i) || errorMessage.match(/HTTP[:\s]+(\d+)/i);
            if (statusMatch) {
